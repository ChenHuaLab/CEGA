...
/* Table of constant values */
 ...
 const doublereal c_b9 = 0.;
 const integer c__1 = 1;
// integer c__9 = 9;
 const integer c__11 = 11;
// integer c__3 = 3;
 const doublereal c_b275 = .001;
 const doublereal c_b276 = .9;
 const doublereal c_b277 = .1;
// integer c__5 = 5;
...
static double __kmin_hj_aux2(double (*func)(double*, void*), int n, double *x1, double *lowbound, double *upbound, void *data, double fx1, double *dx, int *n_calls){
	int k, j = *n_calls;
	double ftmp;
	for (k = 0; k != n; ++k) {
		x1[k] += dx[k];{if(lowbound){x1[k]=x1[k]>lowbound[k]?x1[k]:lowbound[k];}if(upbound){x1[k]=x1[k]<upbound[k]?x1[k]:upbound[k];}}
		ftmp = func(x1, data); ++j;
		if (ftmp < fx1) fx1 = ftmp;
		else { /* search the opposite direction */
			dx[k] = 0.0 - dx[k];
			x1[k] += dx[k] + dx[k];{if(lowbound){x1[k]=x1[k]>lowbound[k]?x1[k]:lowbound[k];}if(upbound){x1[k]=x1[k]<upbound[k]?x1[k]:upbound[k];}}
			ftmp = func(x1, data); ++j;
			if (ftmp < fx1) fx1 = ftmp;
			else{

                x1[k] -= dx[k]; /* back to the original x[k] */

                {if(lowbound){x1[k]=x1[k]>lowbound[k]?x1[k]:lowbound[k];}if(upbound){x1[k]=x1[k]<upbound[k]?x1[k]:upbound[k];}}
...
double kmin_hj2(int n, double *x, void *data, double (*func)(double*, void*), double *lowbound, double *upbound){
	double fx, fx1, *x1, *dx, radius;
	int k, n_calls = 0;
	x1 = (double*)calloc(n, sizeof(double));
	dx = (double*)calloc(n, sizeof(double));
	for (k = 0; k != n; ++k) { /* initial directions, based on MGJ */
		dx[k] = fabs(x[k]) * KMIN_RADIUS;
		if (dx[k] == 0) dx[k] = KMIN_RADIUS;
	}
	radius = KMIN_RADIUS;
	fx1 = fx = func(x, data); ++n_calls;
	for (;;) {
		memcpy(x1, x, n * sizeof(double)); /* x1 = x */
		fx1 = __kmin_hj_aux2(func, n, x1, lowbound, upbound, data, fx, dx, &n_calls);
		while (fx1 < fx) {
			for (k = 0; k != n; ++k) {
				double t = x[k];
			...
			fx = fx1;
			if (n_calls >= KMIN_MAXCALL) break;
			fx1 = func(x1, data); ++n_calls;
			fx1 = __kmin_hj_aux2(func, n, x1, lowbound, upbound, data, fx1, dx, &n_calls);
			if (fx1 >= fx) break;
			for (k = 0; k != n; ++k)
		...
		if (radius >= KMIN_EPS) {
			if (n_calls >= KMIN_MAXCALL) break;
			radius *= KMIN_RADIUS;
			for (k = 0; k != n; ++k) dx[k] *= KMIN_RADIUS;
		} else break; /* converge */
	}
	free(x1); free(dx);
	return fx1;
