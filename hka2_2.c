...
int main(int argc, char **argv){
    ...
    if(afs_file){
        ...
        x[0]=10000;low[0]=100,up[0]=1000000;   //N1
        x[1]=1000;low[1]=100,up[1]=50000;   //T
        ...
        len=sprintf(loginfo, "estimate: like=%.1f\tN1=%.1fT=%.1f\n", like1, x[0], x[1]);
    ...
    double *up=my_new(npara, sizeof(double));
    int *nbd=my_new(npara, sizeof(int));for(i=0;i<npara;i++) nbd[i]=2;
    x[0]=initial_N1;low[0]=initial_N1_low,up[0]=initial_N1_up;   //N1
    x[1]=initial_T;low[1]=initial_T_low,up[1]=initial_T_up;   //T
    ...
    fprintf(stdout, "estimate: like=%.1f\tN1=%.1f\tT=%.1f\n", like1, x[0], x[1]);
    //--
    ...
    if(is_output_true){
        ...
        estimate_exp_jafs(tmp, tmp->arr, tmp->dim1, tmp->dim2, xx[0], xx[0], xx[0], func_n1, func_n2, xx[1], func_mu, 1.0, win_size, NULL, NULL);
        fprintf(stdout, "true    : like=%.1f\tN1=%.1f\tT=%.1f\ts1=%.2f\ts2=%.2f\ts12=%.2f\tD=[%.2f,%.2f,%.2f]\n", like2, xx[0], xx[1], tmp->s1, tmp->s2, tmp->s12, tmp->D1, tmp->D2, tmp->D);
        free(tmp->arr);
        free(tmp);
    }
    ...
    double lambda1_avg=0.0, lambda1_var=0.0;
    for(i=0;i<all_s2s_wins->size;i++){
    ...
    len=sprintf(loginfo, "log_lambda1_avg=%f\tlog_lambda1_var=%f", lambda1_avg, lambda1_var);mylog(loginfo);

    mylog("writing results into file...");
    GzStream *out=gz_stream_open(out_file, "w");
    gz_write(out, loginfo, len);gz_write_char(out, '\n');
    len=sprintf(loginfo, "estimate:\tN1=%f\tT=%f\tlike=%f\n", x[0], x[1], like1);
    gz_write(out, loginfo, len);
    if(is_output_true){
        len=sprintf(loginfo, "true:    \tN1=%f\tT=%f\tlike=%f\n", xx[0], xx[1], like2);
        gz_write(out, loginfo, len);
    }
    ...
    for(i=0;i<to_cul_s2s_index->size;i++){
        ...
        len=sprintf(loginfo, "%s-%d-%d\ts1=%d/%.2f\ts2=%d/%.2f\ts12=%d/[%.2f,%.2f,%.2f]\tD=%d/[%.2f,%.2f,%.2f]\tmu1=%f\tlike1=%f\tmu2=%f\tlambda1=%f\tlike2=%f\tspan_like=%f\tp1=%f\tp2_1=%f\n", win->chr, win->start, win->end, (int)(s2->s1), s2->estimate_s1, (int)(s2->s2), s2->estimate_s2, (int)(s2->s12), s2->s12_1, s2->s12_2, s2->estimate_s12, (int)(s2->D), s2->D1, s2->D2, s2->estimate_D, res_1_mu[index]*func_mu*length, res_1_like[index], res_2_mu[index]*func_mu*length, res_2_lambda1[index], res_2_like[index], span_like, p1, p2_1);
        gz_write(out, loginfo, len);
    }
    gz_stream_destory(out);
...
void print_usage(){
    ...
    printf("Date:     2021-07-09\n");
    printf("Version:  1.0\n\n");
    printf("Usage: hka2 [arguments]\n");
    printf("  input:\n");
...
Win3 *read_win_size_file(char *file){
    ...
    while(gz_read_util(gz1, '\n', line, maxline, &len)){
        ...
        //--
        int num=str_tab_index(line, '\t', tabs, 1);
        if(num!=4){
...
Pop2 *read_hap_file(char *hap_file, char *pos_file){
    ...
    while(gz_read_util(gz1, '\n', line, maxline, &len)){
        ...
        //--
        if(str_tab_index(line, '\t', tabs, 1)!=2){
            fprintf(stderr, "error format(%s)\n", pos_file);
            exit(0);
        }
        i=0;
...
Pop2 *read_tped_file(char *tped_file){
    ...
    while(gz_read_util(gz1, '\n', line, maxline, &len)){
        ...
        if(line[0]=='#') continue;
        //--
        int n_tabs=str_tab_index(line, '\t', tabs, 1);
...
double log_likelihood_afs_func(double *x, void *data){// N0,N1,N2,T
    ...
    estimate_exp_jafs(s2, arr, dim1, dim2, x[0], x[0], x[0], dim1-1, dim2-1, x[1], func_mu, 1.0, global_L, NULL, NULL);
    ...
    fprintf(stdout, "func=%f\tN1=%f\tT=%f\n", -value, x[0], x[1]);
    fflush(stdout);
...
double log_likelihood_func(double *x, void *data){// N0,N1,N2,T
    ...
    if(0){
        ...
        estimate_exp_jafs(s2, arr, dim1, dim2, x[0], x[0], x[0], func_n1, func_n2, x[1], func_mu, 1.0, win_size, NULL, NULL);
        ...
        estimate_exp_jafs(s2, arr, dim1, dim2, x[0], x[0], x[0], func_n1, func_n2, x[1], func_mu, 1.0, global_L, NULL, NULL);
...
double log_likelihood_mu_lambda_func(double *x, void *data){//mu lambda1 lambda2
    ...
    estimate_exp_jafs(tmp, tmp->arr, tmp->dim1, tmp->dim2, global_N0, global_N1*x[1], global_N2, func_n1, func_n2, global_T, func_mu*x[0], 1.0, (double)length, NULL, NULL);
...
void optimize_mu_lambda_for_win(int index){
    ...
    Win1 *win=all_s2s_wins->elementData[index];
    int length=win->length==0 ? (win->end-win->start):win->length;
    fprintf(stdout, "estimate_mu_lambda: %s-%d-%d\tmu=%f\tlambda1=%f\tlike=%f\n", win->chr, win->start, win->end, res_2_mu[index]*func_mu*length, res_2_lambda1[index], res_2_like[index]);
}
